...
type FinanceView = 'overview' | 'revenue' | 'expenses' | 'recurring';
...
export function Finance() {
  ...
  const [trafficPaymentModalOpen, setTrafficPaymentModalOpen] = useState(false);
  ...
  const [trafficPaymentDraft, setTrafficPaymentDraft] = useState({ amount: 0, paidDate: getToday(), notes: '' });
  ...
  const groupedTrafficExpensesInPeriod = useMemo(() => groupTrafficByDay(adjustedTrafficExpensesInPeriod), [adjustedTrafficExpensesInPeriod]);
  const filteredExpensesByDate = useMemo(() => [...groupedTrafficExpensesInPeriod, ...manualExpensesInPeriod], [groupedTrafficExpensesInPeriod, manualExpensesInPeriod]);
  ...
  const overdue = filteredExpensesByDate.filter((expense) => expense.status === 'Aberta' && isExpenseOverdue(expense));
  ...
  const filteredExpenses = useMemo(() => {
    ...
    return filteredExpensesByDate.filter((expense) => {
      ...
      const matchesView = view === 'overview' || view === 'expenses' || (view === 'recurring' && expense.isRecurring);
  ...
  const saveExpense = async () => {
    ...
    await saveExpenseDraft(expenseDraft, editingExpense?.id);
    setExpenseModalOpen(false);
  };

  const saveTrafficPayment = async () => {
    if (trafficPaymentDraft.amount <= 0) return;
    const paidDate = trafficPaymentDraft.paidDate || getToday();
    await saveExpenseDraft({
      ...emptyExpenseDraft,
      description: `Pagamento de tráfego - ${formatDate(paidDate)}`,
      category: 'Tráfego',
      kind: 'Variável',
      amount: Number(trafficPaymentDraft.amount),
      dueDate: paidDate,
      paidDate,
      status: 'Paga',
      paymentMethod: 'Pagamento manual',
      notes: trafficPaymentDraft.notes || 'Abate automaticamente o tráfego aberto mais antigo.',
    });
    setTrafficPaymentDraft({ amount: 0, paidDate: getToday(), notes: '' });
    setTrafficPaymentModalOpen(false);
  };

  const saveExpenseDraft = async (draft: ExpenseDraft, editingId?: string) => {
    const normalized = {
      ...draft,
      amount: Number(draft.amount),
      paidDate: draft.status === 'Paga' ? draft.paidDate || new Date().toISOString().slice(0, 10) : '',
    };

    if (editingId) {
      await persistExpensePatch(editingId, normalized);
    ...
    <div className="mx-auto max-w-[1500px] space-y-6 p-10 text-slate-300">
      <section className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={() => setTrafficPaymentModalOpen(true)} className="inline-flex items-center gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-[11px] font-bold text-emerald-200 transition-colors hover:bg-emerald-500/15">
      ...
      <section className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-6">
        <MetricCard label="Entradas por vendas" value={money.format(totalRevenue)} detail={`bruto ${money.format(grossSalesRevenue)} - diferença ${money.format(platformFees)}`} icon={TrendingUp} tone="blue" />
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-12">
        <div className="space-y-6 xl:col-span-8">
          <ExpensePanel
            ...
            onPayTraffic={() => setTrafficPaymentModalOpen(true)}
          />
        </div>
      ...
      {trafficPaymentModalOpen && (
        <TrafficPaymentModal
          draft={trafficPaymentDraft}
          openAmount={trafficOpenTotal}
          onChange={setTrafficPaymentDraft}
          onClose={() => setTrafficPaymentModalOpen(false)}
          onSave={saveTrafficPayment}
        />
      )}
  ...
  return (
    <div className="rounded-2xl border border-slate-900/50 bg-slate-950 p-5">
      <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 className="text-sm font-semibold text-slate-200">{showRevenue ? 'Entradas por vendas' : view === 'recurring' ? 'Contas fixas' : 'Saídas por categoria'}</h2>
          <p className="mt-1 text-[11px] text-slate-500">{showRevenue ? 'Comissões vindas das planilhas no período.' : 'Resumo concentrado das saídas. O tráfego aparece agrupado por dia e pode ser pago parcialmente.'}</p>
        </div>
        <div className="flex w-full flex-col gap-2 lg:w-auto lg:flex-row lg:items-center">
          <ViewSwitcher value={view} onChange={onView} />
          {!showRevenue && <button type="button" onClick={onPayTraffic} className="inline-flex h-9 items-center gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 text-[11px] font-bold text-emerald-200 transition-colors hover:bg-emerald-500/15"><CheckCircle2 size={13} />Pagar tráfego</button>}
      ...
      {!showRevenue && <div className="mb-4 grid grid-cols-1 gap-3 rounded-xl border border-slate-900 bg-slate-950/70 p-3 md:grid-cols-5">
...
function ExpenseRow({ expense, onEdit, onDelete, onTogglePaid }: { expense: FinanceExpense; onEdit: (expense: FinanceExpense) => void; onDelete: (expense: FinanceExpense) => void; onTogglePaid: (expense: FinanceExpense) => void }) {
  return (
    <div className="grid grid-cols-[1.5fr_0.8fr_0.8fr_0.8fr_128px] items-center gap-4 px-4 py-3">
      ...
      <span className={cn('text-[11px]', isExpenseOverdue(expense) && expense.status === 'Aberta' ? 'text-rose-300' : 'text-slate-500')}>{expense.source === 'traffic' ? `vence ${formatDate(getTrafficBillingDueDate(expense.dueDate))}` : formatDate(expense.dueDate)}</span>
...
function RevenueList({ revenue }: { revenue: SalesRevenuePoint[] }) {
  return (
    <div className="overflow-hidden rounded-xl border border-slate-900">
      {revenue.length ? revenue.slice().reverse().map((item, index) => (
        <div key={`${item.date}-${index}`} className="grid grid-cols-[1fr_auto] items-center gap-4 border-b border-slate-900 px-4 py-3 last:border-b-0 md:grid-cols-[1fr_0.7fr_auto]">
          <div>
            <p className="text-[12px] font-semibold text-slate-200">{item.platform}</p>
            <p className="mt-1 text-[10px] uppercase tracking-wider text-slate-600">{formatDate(item.date)} · {item.orders} venda(s)</p>
          </div>
          <span className="hidden text-[11px] text-slate-500 md:block">Comissão da planilha</span>
          <span className="font-mono text-[13px] font-bold text-emerald-300">{money.format(item.netRevenue ?? item.revenue)}</span>
        </div>
      )) : <div className="px-4 py-10 text-center text-[12px] text-slate-500">Nenhuma entrada no período.</div>}
    </div>
  );
}

function TrafficPaymentModal({ draft, openAmount, onChange, onClose, onSave }: { draft: { amount: number; paidDate: string; notes: string }; openAmount: number; onChange: (draft: { amount: number; paidDate: string; notes: string }) => void; onClose: () => void; onSave: () => void }) {
  return (
    <Modal title="Pagar tráfego" onClose={onClose}>
      <div className="mb-4 rounded-xl border border-emerald-500/15 bg-emerald-500/[0.04] p-3 text-[11px] leading-5 text-emerald-100">
        Informe quanto foi pago. O valor abate automaticamente o tráfego aberto mais antigo até chegar nos dias mais recentes. Em aberto agora: <span className="font-mono font-bold">{money.format(openAmount)}</span>.
      </div>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <Field label="Valor pago"><input className={inputClass} type="number" min={0} step="0.01" value={draft.amount || ''} onChange={(event) => onChange({ ...draft, amount: Number(event.target.value) })} autoFocus /></Field>
        <Field label="Data do pagamento"><input className={inputClass} type="date" value={draft.paidDate} onChange={(event) => onChange({ ...draft, paidDate: event.target.value })} /></Field>
        <Field label="Observações" className="md:col-span-2"><textarea className={cn(inputClass, 'h-20 py-2')} value={draft.notes} onChange={(event) => onChange({ ...draft, notes: event.target.value })} /></Field>
      </div>
      <div className="mt-5 flex justify-end gap-2">
        ...
        <button onClick={onSave} disabled={draft.amount <= 0} className="rounded-lg bg-emerald-600 px-3 py-2 text-[11px] font-bold text-white transition-colors hover:bg-emerald-500 disabled:cursor-not-allowed disabled:opacity-40">Registrar pagamento</button>
      </div>
    </Modal>
  );
}
...
function ViewSwitcher({ value, onChange }: { value: FinanceView; onChange: (value: FinanceView) => void }) {
  const options: Array<{ id: FinanceView; label: string; icon: typeof Wallet }> = [
    ...
    { id: 'revenue', label: 'Entradas', icon: TrendingUp },
...
function groupTrafficByDay(expenses: FinanceExpense[]) {
  const groups = expenses.reduce<Record<string, FinanceExpense>>((acc, expense) => {
    const date = expense.dueDate || getToday();
    const current = acc[date];
    if (!current) {
      acc[date] = {
        ...expense,
        id: `traffic-day-${date}`,
        description: `Tráfego importado - ${formatDate(date)}`,
        dueDate: date,
        amount: roundCurrency(expense.amount),
        rawAmount: roundCurrency(expense.rawAmount ?? 0),
        taxAmount: roundCurrency(expense.taxAmount ?? 0),
        notes: 'Total diário importado das planilhas de anúncios.',
      };
      return acc;
    }
    current.amount = roundCurrency(current.amount + expense.amount);
    current.rawAmount = roundCurrency((current.rawAmount ?? 0) + (expense.rawAmount ?? 0));
    current.taxAmount = roundCurrency((current.taxAmount ?? 0) + (expense.taxAmount ?? 0));
    return acc;
  }, {});

  return Object.values(groups).sort((first, second) => first.dueDate.localeCompare(second.dueDate));
}

function getTrafficBillingDueDate(date: string) {
  if (!date) return getToday();
  const source = new Date(`${date}T00:00:00`);
  if (Number.isNaN(source.getTime())) return getToday();
  return new Date(source.getFullYear(), source.getMonth() + 1, 15).toISOString().slice(0, 10);
}

function isExpenseOverdue(expense: FinanceExpense) {
  return isOverdue(expense.source === 'traffic' ? getTrafficBillingDueDate(expense.dueDate) : expense.dueDate);
}
