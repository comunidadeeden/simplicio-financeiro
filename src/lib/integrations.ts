...
export interface CreativeDriveConfig {
  name: string;
  folderUrl: string;
  status: ConnectionStatus;
}
...
export interface IntegrationSettings {
  ...
  quizSources: GenericSheetConfig[];
  creativeDrive: CreativeDriveConfig;
  cycle: CycleSettings;
...
export const defaultCreativeDrive: CreativeDriveConfig = {
  name: 'Criativos do Drive',
  folderUrl: 'https://drive.google.com/drive/folders/1OESwM3FY19s430ljQVyizDTPL9nytr64?usp=sharing',
  status: 'Ativa',
};
...
export const defaultIntegrationSettings: IntegrationSettings = {
  ...
  liveSources: [],
  quizSources: [],
  creativeDrive: defaultCreativeDrive,
...
export async function loadSheetData(settings: IntegrationSettings): Promise<SheetLoadResult> {
  ...
  const quizLists = await Promise.all(
    settings.quizSources
      .filter((source) => source.status === 'Ativa' && source.spreadsheetUrl.trim())
      .map((source) => loadGenericSheet(source).catch((error) => {
        errors.push(getErrorMessage(source.name || 'Quiz', error));
        return [] as Record<string, string>[];
      })),
  );
...
async function loadSales(config: SalesSheetConfig): Promise<SalesRevenuePoint[]> {
  ...
  return rows.map((row, index) => {
    ...
    const commissionRevenue = parseMoney(readColumn(row, '', ['valor comissão', 'valor comissao', 'comissão', 'comissao', 'valor líquido', 'valor liquido']));
    const configuredRevenue = parseMoney(readColumn(row, config.revenueColumn, ['valor venda', 'valor da venda', 'valor', 'receita', 'faturamento', 'total']));
    const revenue = commissionRevenue || configuredRevenue;
    const grossRevenue = parseMoney(readColumn(row, '', ['valor venda', 'valor da venda', 'valor', 'receita', 'faturamento', 'total'])) || revenue;
    const platformFeeAmount = grossRevenue && commissionRevenue ? roundCurrency(Math.max(grossRevenue - commissionRevenue, 0)) : 0;
    const platformFeeRate = grossRevenue ? platformFeeAmount / grossRevenue : normalizeRate(config.platformFeeRate);
...
const salesColumnAliases = {
  ...
  revenue: ['valor comissão', 'valor comissao', 'comissão', 'comissao', 'valor líquido', 'valor liquido', 'valor venda', 'valor da venda', 'valor', 'receita', 'faturamento', 'total', 'amount', 'price', 'preço', 'preco', 'sale amount'],
...
function mergeSettings(settings: Partial<IntegrationSettings>): IntegrationSettings {
  return {
    ...
    quizSources: normalizeGenericSources(settings.quizSources),
    creativeDrive: { ...defaultCreativeDrive, ...settings.creativeDrive },
    cycle: { ...defaultCycleSettings, ...settings.cycle },
    webhook: normalizeWebhookSettings(settings.webhook),
  };
}

function normalizeWebhookSettings(webhook?: Partial<WebhookConfig>): WebhookConfig {
  const legacyTime = webhook?.cadence?.match(/(\d{2}:\d{2})/)?.[1];
  const scheduleTimes = Array.from(new Set([
    ...(webhook?.scheduleTimes?.length ? webhook.scheduleTimes : []),
    ...(legacyTime ? [legacyTime] : []),
    ...(!webhook?.scheduleTimes?.length && !legacyTime ? defaultIntegrationSettings.webhook.scheduleTimes : []),
  ])).sort();

  return {
    ...defaultIntegrationSettings.webhook,
    ...webhook,
    url: webhook?.url || defaultIntegrationSettings.webhook.url,
    cadence: webhook?.cadence || defaultIntegrationSettings.webhook.cadence,
    scheduleTimes,
  };
}
